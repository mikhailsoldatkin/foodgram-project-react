...
- Скопировать на сервер файлы docker-compose.yaml, nginx.conf из папки infra (команды выполнять находясь в корневой директории проекта):

```
scp -r infra/* username@IP:/home/username/     # username - имя пользователя на сервере, IP - публичный IP сервера
```

Для работы с GitHub Actions необходимо:

- В репозитории На GitHub в разделе Secrets > Actions создать переменные окружения:
```
